...
class Snake:
    ...
    def reset(self):
        ...
        self.__body : list = [(self.__x,self.__y)] * 50
    ...
    def update(self, direction):
        ...
        new_pos = (self.__x, self.__y)
        last_pos = self.__body[-1]

        # Interpoler if speed > 1
        interpolated_positions = []
        for i in range(1, self.__speed + 1):
            interp_x = int(last_pos[0] + (new_pos[0] - last_pos[0]) * i / self.__speed)
            interp_y = int(last_pos[1] + (new_pos[1] - last_pos[1]) * i / self.__speed)
    ...
    def checkCollision(self, screen_width, screen_height):
        collision = False
